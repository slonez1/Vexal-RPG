...
def get_effective_attributes():
    ...
    # 2. Apply Condition Modifiers (e.g., Vexal Active -2 to ALL)
...
def get_gm_response(user_input):
    # This maintains the narrative loop
    return f"The GM acknowledges: '{user_input}'. Your modified stats and level {gs['level']} influence the outcome."
...
with st.sidebar:
    ...
    st.divider()
    # CRITICAL FIX: Sidebar now uses 'eff' for rendering
    eff = get_effective_attributes()
    st.markdown("<div style='text-align:center; font-size:0.7rem; font-weight:bold; color:#888;'>EFFECTIVE ATTRIBUTES</div>", unsafe_allow_html=True)
    a_cols = st.columns(3)
    for i, (attr, base) in enumerate(gs['attributes'].items()):
        val = eff[attr]
        diff = val - base
        # Color based on whether stat is buffed or penalized
        color = "#ff4b4b" if diff < 0 else "#28a745" if diff > 0 else "#eee"
        val_display = f"{val} ({diff})" if diff != 0 else f"{val}"
        with a_cols[i%3]:
            st.markdown(f"""<div class="attr-box"><div class="attr-label">{attr}</div><div class="attr-val" style="color:{color};">{val_display}</div></div>""", unsafe_allow_html=True)
...
# --- 4. MAIN TABS & CONSOLE (PERSISTENCE AUDIT: VERIFIED) ---
...
with tab_con:
    ...
    with col3:
        imp_text = st.text_input("Impromptu", placeholder="Define action...", label_visibility="collapsed")
        if st.button("🚀 Execute Impromptu", use_container_width=True):
            if imp_text:
                ...
                st.session_state.messages.append({"role": "assistant", "content": get_gm_response(full_imp)})
                st.rerun()
    ...
    if direct_cmd:
        ...
        st.session_state.messages.append({"role": "assistant", "content": get_gm_response(final_msg)})
...
with tab_stat:
    ...
    sc = st.columns(3)
    # Corrected: Saving throws now use effective attributes
